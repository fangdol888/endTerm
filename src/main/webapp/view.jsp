...
<%
	...
	<div class="container mt-4">
		<div class="row">
			...
			<div class="container mt-4">
			<a href="bbs.jsp" class="btn btn-primary float-right mr-1">목록</a>
			
			<!-- 해당 글의 작성자가 본인 또는 관리자라면 수정과 삭제가 가능하도록 코드 추가 -->
			<%
				if(userID != null && (userID.equals(bbs.getUserID()) || Integer.parseInt(String.valueOf(session.getAttribute("auth"))) != 0) ){
			%>
					<a href="update.jsp?bbsID=<%= bbsID %>" class="btn btn-primary float-right mr-1">수정</a>
					<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="deleteAction.jsp?bbsID=<%= bbsID %>" class="btn btn-primary float-right mr-1">삭제</a>
			<%
				}
			%>
			</div>
